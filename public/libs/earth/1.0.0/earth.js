...
(function () {
    ...
    let MOVE_END_WAIT = 1000;                 // time to wait for a move operation to be considered done (millis)
    ...
    let PARTICLE_LINE_WIDTH = 1.0;            // line width of a drawn particle
    let PARTICLE_MULTIPLIER = 7;              // particle count scalar (completely arbitrary--this values looks nice)
    let PARTICLE_REDUCTION = 0.75;            // reduce particle count to this much of normal for mobile devices
    let FRAME_RATE = 40;                      // desired milliseconds per frame
    ...
    let configuration = µ.buildConfiguration(globes, products.overlayTypes);  // holds the page's current configuration settings
    ...
    function buildInputController() {
        ...
        let zoom = d3.behavior.zoom().on("zoomstart", function () {
    ...
    function interpolateField(globe, grids) {
        ...
        (function batchInterpolate() {
            try {
                ...
                let field = createField(columns, bounds, mask);
                d.resolve(field);
            } catch (e) {
                d.reject(e);
            }
    ...
    function init() {
        ...
        d3.select("#ocean-mode-enable").on("click", function () {
            if (configuration.get("param") !== "ocean") {
                ...
                let ocean = {param: "ocean", surface: "surface", level: "currents", overlayType: "default"};
